...
def get_user_from_db(user_id: int) -> Dict:
    ...
    cursor.execute(
        "SELECT id, name, age, sex, height, weight, insurance_provider, insurance_policy, allergies, medical_history, profile_pic_path, profile_pic_path2 "
        "FROM users WHERE id = ?",
        (user_id,)
    )
    row = cursor.fetchone()
    ...
    if row:
        return {
            ...
            "profile_pic_path2": row[11] if len(row) > 11 else None
        }
    else:
        return None
...
async def add_user(
    ...
    profile_pic2: UploadFile = File(...)
):
    # Read first photo data
    pic_data = await profile_pic.read()
    
    # Save first photo to profile_pics folder
    pic_filename = f"{name}_1_{profile_pic.filename}"
    pic_path = os.path.join(PROFILE_PICS_DIR, pic_filename)
    
    with open(pic_path, "wb") as f:
        f.write(pic_data)
    
    # Also save first photo to known_faces folder for face recognition
    # Get file extension from original filename
    file_ext = os.path.splitext(profile_pic.filename)[1] or ".jpg"
    known_face_filename = f"{name.lower()}_1{file_ext}"
    known_face_path = os.path.join(KNOWN_FACES_DIR, known_face_filename)
    
    with open(known_face_path, "wb") as f:
        f.write(pic_data)
    
    # Save second photo to profile_pics folder only
    pic_filename2 = f"{name}_2_{profile_pic2.filename}"
    pic_path2 = os.path.join(PROFILE_PICS_DIR, pic_filename2)
    
    with open(pic_path2, "wb") as f:
        f.write(await profile_pic2.read())
    
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    
    # Check if profile_pic_path2 column exists, if not add it
    cursor.execute("PRAGMA table_info(users)")
    columns = [column[1] for column in cursor.fetchall()]
    
    if 'profile_pic_path2' not in columns:
        cursor.execute("ALTER TABLE users ADD COLUMN profile_pic_path2 TEXT")
    
    cursor.execute("""
        INSERT INTO users (name, age, sex, height, weight, insurance_provider, insurance_policy, allergies, medical_history, profile_pic_path, profile_pic_path2)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (name, age, sex, height, weight, insurance_provider, insurance_policy, allergies, medical_history, pic_path, pic_path2))
    
    user_id = cursor.lastrowid
    conn.commit()
    conn.close()
    
    # Reload known faces to include the newly added face
    load_known_faces()
...
def startup_event():
    load_known_faces()
    print("Backend server started with all endpoints")